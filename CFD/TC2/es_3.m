%es_3
ni=1e-6;
K=0.41;
E=8.6;

x_tg=780;

u_tau=sqrt(STRS(x_tg,1));
U=U1(x_tg,:)./u_tau;
delta_ni=ni./u_tau;
y_plus=Y_C/delta_ni;
u_plus=1/K*log(E*y_plus);


figure
plot(y_plus,u_plus,'r','LineWidth',3)
grid on
xline(30,'b','LineWidth',3)
title('u^{+} vs y^{+}')

figure
semilogx(y_plus, u_plus,'r')
grid on
hold on
semilogx(y_plus, U,'b*');
xline(30,'k','LineWidth',3)
xline(130,'k','LineWidth',3)
legend('Theoretical','Experimental')
title('Theoretical Solution vs Experimental Solution')


%es 2
U0=U1(x_tg,end);
c_f_exp=2*u_tau.^2./(U0^2);
Re=2*0.05*1/ni;
B=5.2;
B1=0.5;

tosolve=@(x) (sqrt(2/x)-1/K*log(Re*(sqrt(8/x)-4.8).^(-1))-B-B1);
exact_sol=fzero(tosolve, c_f_exp);

residual=abs(sqrt(2/c_f_exp)-1/K*log(Re*(sqrt(8/c_f_exp)-4.8).^(-1))-B-B1);